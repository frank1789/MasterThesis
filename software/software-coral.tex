...
mutex is always well-defined. \tetxttt{QMutex} Locker should be created within a
function where a \tetxttt{QMutex} needs to be locked. The mutex is locked when
\tetxttt{QMutexLocker} is created. If locked, the mutex will be unlocked when
the \tetxttt{QMutex} Locker is destroyed.
Using \tetxttt{QMutexLocker} greatly simplifies the code, and makes it more
